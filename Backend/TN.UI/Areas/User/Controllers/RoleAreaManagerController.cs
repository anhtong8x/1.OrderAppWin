...
namespace TN.UI.Areas.Manager.Controllers
{
    ...
    public class RoleAreaManagerController : Controller
    {
        ...
        public async Task<IActionResult> IndexPost(int? page, int? limit, string key, string ordertype, string orderby)
        {
            ...
            var data = await _iRoleAreaRepository.SearchPagedList(
            page ?? 1,
            limit ?? 10,
        ...
        public async Task<ResponseModel> CreatePost(RoleAreaModel use, string returnurl)
        {
            try
            {
        ...
        {
            var dl = await _iRoleAreaRepository.SearchOneAsync(m => m.Id == id);
            if (dl == null)
            {
                return View("404");
            }
        ...
        public async Task<ResponseModel> EditPost(RoleAreaModel use, string id, string returnurl)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        public async Task<ResponseModel> DeletePost(string id)
        {
            try
            {
                ...
                var kt = await _iRoleAreaRepository.SearchOneAsync(m => m.Id == id);
                if (kt == null)
                {
                    return new ResponseModel() { Output = 0, Message = "Dữ liệu không tồn tại, vui lòng thử lại.", Type = ResponseTypeMessage.Warning };
                }
