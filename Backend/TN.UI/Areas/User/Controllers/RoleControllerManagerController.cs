...
namespace TN.UI.Areas.Manager.Controllers
{
    ...
    public class RoleControllerManagerController : Controller
    {
        ...
        {
            ViewData["RoleGroupSelectListItem"] = new SelectList(await _iRoleGroupRepository.Search(x => x.Status == true), "Id", "Name");
            ViewData["RoleAreaSelectList"] = new SelectList(await _iRoleAreaRepository.Search(x => x.Status == true), "Id", "Name");
            return View();
        }
        ...
        public async Task<IActionResult> IndexPost(int? page, int? limit, string key, string ordertype, string orderby, int? groupId, string areaId)
        {
            ...
            var data = await _iRoleControllerRepository.SearchPagedList(
            page ?? 1,
            limit ?? 10,
        ...
        public async Task<IActionResult> Create()
        {
            ...
            {
                RoleGroupSelectList = new SelectList(await _iRoleGroupRepository.Search(x => x.Status == true), "Id", "Name"),
                RoleAreaSelectList = new SelectList(await _iRoleAreaRepository.Search(x => x.Status == true), "Id", "Name")
            });
        }
        ...
        {
            var dl = await _iRoleControllerRepository.SearchOneAsync(m => m.Id == id);
            if (dl == null)
            {
                return View("404");
            }
            var model = MapModel<RoleControllerModel>.Go(dl);
            model.RoleGroupSelectList = new SelectList(await _iRoleGroupRepository.Search(x => x.Status == true), "Id", "Name");
            model.RoleAreaSelectList = new SelectList(await _iRoleAreaRepository.Search(x => x.Status == true), "Id", "Name");
            return View(model);
        }
        ...
        public async Task<ResponseModel> DeletePost(string id)
        {
            try
            {
                ...
                var kt = await _iRoleControllerRepository.SearchOneAsync(m => m.Id == id);
                if (kt == null)
                {
                    return new ResponseModel() { Output = 0, Message = "Dữ liệu không tồn tại, vui lòng thử lại.", Type = ResponseTypeMessage.Warning };
                }
                ...
                await _iRoleControllerRepository.Commit();
                return new ResponseModel() { Output = 1, Message = "Xóa dữ liệu thành công.", Type = ResponseTypeMessage.Success };
            }
            catch (Exception ex)
            {
                _logger.LogError(LoggingEvents.GENERATE_ITEMS, "#Trong-[Log]{0}", ex);
