...
namespace TN.UI.Areas.Manager.Controllers
{
    ...
    public class RoleActionManagerController : BaseController
    {
        ...
        {
            controllerName = "RoleActionManager";
        ...
        {
            var dl = await _iRoleActionRepository.SearchOneAsync(m => m.Id == id,x=>x.RoleController);
            if (dl == null)
            {
                return View("404");
            }
        ...
        public async Task<ResponseModel> EditPost(RoleActionModel use, int id, string returnurl)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    var dl = await _iRoleActionRepository.SearchOneAsync(m => m.Id == id);
                    if (dl == null)
                    {
                        return new ResponseModel() { Output = 0, Message = "Dữ liệu không tồn tại, vui lòng thử lại.", Type = ResponseTypeMessage.Warning };
                    }
        ...
        public async Task<ResponseModel> DeletePost(int id)
        {
            try
            {
                ...
                var kt = await _iRoleActionRepository.SearchOneAsync(m => m.Id == id);
                if (kt == null)
                {
                    return new ResponseModel() { Output = 0, Message = "Dữ liệu không tồn tại, vui lòng thử lại.", Type = ResponseTypeMessage.Warning };
                }
        ...
        {
            var data = await _iRoleControllerRepository.SearchAdvanced(m => m.AreaId == areaId, x => x.OrderBy(m => m.Name));
