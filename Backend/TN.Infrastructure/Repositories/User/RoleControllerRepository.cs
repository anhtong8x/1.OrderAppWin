...
namespace TN.Infrastructure.Repositories
{
    ...
    public class RoleControllerRepository : EntityBaseRepository<RoleController>, IRoleControllerRepository
    {
        ...
        public virtual async Task<BaseLogDataModel<string>> UpdateRoleAsync(int roleId,List<int> ids)
        {
            var listRole =await _context.RoleDetails.Where(m => m.RoleId == roleId).ToListAsync();
            var dlFirst = Newtonsoft.Json.JsonConvert.SerializeObject(listRole);
            ...
            var listLast = await _context.RoleDetails.Where(m => m.RoleId == roleId).ToListAsync();
            return new BaseLogDataModel<string> {DataBefore = dlFirst, DataAfter= Newtonsoft.Json.JsonConvert.SerializeObject(listLast) };
        }
